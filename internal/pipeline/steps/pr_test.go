...
func TestPRStep_LocalFetchOriginCreatesGitHubTargetPR(t *testing.T) {
	...
	var stepLogs []string
	sctx.Log = func(line string) { stepLogs = append(stepLogs, line) }
	if got := resolvePushURL(sctx); got != sctx.Repo.ForkURL {
		t.Fatalf("push target = %q, want %q", got, sctx.Repo.ForkURL)
	}
	if got := providerURL(sctx); got != sctx.Repo.ForkURL {
		t.Fatalf("provider target = %q, want %q", got, sctx.Repo.ForkURL)
	}
	if got := scm.DetectProviderContext(sctx.Ctx, providerURL(sctx)); got != scm.ProviderGitHub {
		t.Fatalf("provider = %q, want %q", got, scm.ProviderGitHub)
	}
	...
	if outcome == nil || outcome.Skipped {
		t.Fatalf("PR step skipped unexpectedly: outcome=%#v logs=%v", outcome, stepLogs)
	}
	logData, err := os.ReadFile(logFile)
	if err != nil {
		t.Fatal(err)
