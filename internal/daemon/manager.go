...
// cannot neutralize project agent-instruction files are dropped from the launch set rather
		...
		if len(created) == 0 {
			if cfg.DisableProjectSettings && len(dropped) > 0 {
				return nil, fmt.Errorf("gate agent set does not neutralize the target repository's project "+
					...
					"neutralization knob under disable_project_settings; add a verified "+
					"subscription_agents candidate or select a verified agent directly", strings.Join(dropped, ", "))
			}
