...
// so codex wins while the Pi candidates remain on the ordered route.
...
// TestNewPipelineAgent_SubscriptionOptOut_KeepsVerifiedCandidates proves the
// subscription filter follows each adapter's effective neutralization capability.
// Pi-backed Kimi/Grok remain eligible now that Pi has a verified suppression flag.
func TestNewPipelineAgent_SubscriptionOptOut_KeepsVerifiedCandidates(t *testing.T) {
	...
	if cfg.SubscriptionRoute == nil || len(cfg.SubscriptionRoute.Ordered) != 3 {
		t.Fatalf("opt-out must keep every neutralizing candidate on the launch route, got %+v", cfg.SubscriptionRoute)
	}
	want := []string{"codex", "kimi", "grok"}
	for i, name := range want {
		if cfg.SubscriptionRoute.Ordered[i].Name != name {
			t.Fatalf("ordered[%d] = %q, want %q (route=%+v)", i, cfg.SubscriptionRoute.Ordered[i].Name, name, cfg.SubscriptionRoute)
		}
	}
	...
	for _, name := range want {
		if !strings.Contains(persistedRoute, `"name":"`+name+`"`) {
			t.Fatalf("persisted recovery route must contain %s, got %s", name, persistedRoute)
		}
	}
	if !agent.SupportsSessionProvider(ag, "codex") {
...
// proves a subscription route containing only an unverified adapter still fails closed.
func TestNewPipelineAgent_SubscriptionOptOut_RefusesWhenOnlyUnverifiedCandidatesRemain(t *testing.T) {
	...
	cfg := &config.Config{
		...
		SubscriptionAgents: config.SubscriptionAgentsConfig{
			Candidates: []config.SubscriptionCandidate{
				{Name: "open", Agent: types.AgentOpenCode, QuotaProvider: "open"},
			},
		},
		QuotaFetch: func(context.Context, string, []string) (*quota.Snapshot, error) {
			return &quota.Snapshot{
				...
				Providers: []quota.ProviderSnapshot{
					{
						...
						QuotaSemantics: quota.QuotaSemantics{
							Status: "known",
							EffectiveAvailability: []quota.EffectiveAvailability{{
								Scope: "all_models", Status: "known", EffectivePercentRemaining: &remaining, Pace: &quota.Pace{Status: "behind"},
							}},
						},
					},
				},
	...
	if err == nil {
		t.Fatal("unverified-only subscription route must be refused under disable_project_settings")
	}
	msg := err.Error()
	for _, want := range []string{"does not neutralize", "open", "opencode"} {
...
// TestNewPipelineAgent_SubscriptionNoOptOut_KeepsFallbacks is the
// backward-compat guarantee: without disable_project_settings, all candidates
// remain on the subscription fallback route exactly as before.
func TestNewPipelineAgent_SubscriptionNoOptOut_KeepsFallbacks(t *testing.T) {
