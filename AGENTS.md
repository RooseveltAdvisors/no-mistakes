...
- Socket death is not process exit: the lock is released only when the daemon process exits. `Stop` captures the live daemon's pid record before requesting shutdown and `waitForDaemonStop` reports "stopped" only once that exact process is gone, so `daemon restart` cannot launch the next daemon into a still-held lock (which makes the child exit 1 immediately). An in-process daemon recording our own pid keeps the socket-only semantics. That wait is only possible because `stopDetachedDaemon` closes the connection carrying the shutdown request before waiting: the daemon's accept loop drains in-flight connections (`ipc` `wg.Wait`) before its process can exit, so holding it open deadlocks the wait against that exit. Regressions: `TestWaitForDaemonStopWaitsForSingletonLockRelease`, `TestStopDetachedDaemonReleasesShutdownConnectionBeforeWaiting`.
...
- `document.instructions` (the repo's documentation placement policy) and `disable_project_settings` (the gate-agent project-instruction opt-out) are also trusted-only: a pushed branch must not weaken either boundary. When the opt-out is enabled, only adapters with verified effective suppression may launch; `createResolvedPipelineAgents` drops non-neutralizing `subscription_agents` candidates (for example Pi-backed Kimi/Grok) so a verified native Codex/Claude primary still runs, while explicit ordered fallback lists still fail closed if any member is unverified. Non-executing fields (`ignore_patterns`, `auto_fix`, `commit`, `intent`, `test`) are still read from the pushed branch.
